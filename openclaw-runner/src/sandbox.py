...
Creates isolated workspace directories for each session using
POSIX permissions. Integrates with EFS for persistence.
"""
...
class SandboxConfig:
    ...
    dir_permissions: int = 0o700  # Only owner can access
...
class Sandbox:
    ...
    def create(self, session_id: str) -> str:
        ...
        Raises:
            SandboxError: If workspace cannot be created
        """
    ...
    def destroy(self, session_id: str) -> None:
        ...
        Raises:
            SandboxError: If workspace cannot be destroyed
        """
        workspace_path = self._session_path(session_id)
        ...
        try:
            ...
            logger.info(f"Destroyed sandbox workspace: {workspace_path}")
        except OSError as e:
    ...
    def get_path(self, session_id: str) -> Optional[str]:
        """Get path to existing session workspace.

        Args:
            session_id: Unique session identifier

        Returns:
            Absolute path to workspace if exists, None otherwise
        """
        workspace_path = self._session_path(session_id)
        if workspace_path.exists():
            return str(workspace_path)
    ...
    def list_sessions(self) -> list[str]:
        ...
        Returns:
            List of session IDs with active workspaces
        """
        root = Path(self.config.workspace_root)
        if not root.exists():
            return []
    ...
    def cleanup_stale(self, active_sessions: set[str]) -> int:
        """Remove workspaces for sessions no longer active.
