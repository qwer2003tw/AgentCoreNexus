...
class MemoryAuditLogger:
    ...
    def log_memory_access(
        ...
        if success:
            logger.info(f"Memory operation: {operation}", extra=audit_log)
        else:
            logger.warning(f"Memory operation failed: {operation}", extra=audit_log)

    @staticmethod
    def log_session_created(user_id: str, actor_id: str, session_id: str, memory_id: str):
        """記錄 Session 創建"""
        MemoryAuditLogger.log_memory_access(
            operation="create_session",
            user_id=user_id,
    ...
    @staticmethod
    def log_session_failed(user_id: str, actor_id: str, session_id: str, error: str):
        """記錄 Session 創建失敗"""
        MemoryAuditLogger.log_memory_access(
            operation="create_session",
            user_id=user_id,
    ...
    @staticmethod
    def log_memory_retrieval(user_id: str, actor_id: str, session_id: str, retrieved_count: int):
        """記錄記憶檢索"""
        MemoryAuditLogger.log_memory_access(
    ...
    def log_security_event(
        ...
        if severity in ["high", "critical"]:
            logger.error(f"Security event: {event_type} - {description}", extra=security_log)
        elif severity == "medium":
            logger.warning(f"Security event: {event_type} - {description}", extra=security_log)
        else:
            logger.info(f"Security event: {event_type} - {description}", extra=security_log)
    ...
    @staticmethod
    def log_suspicious_activity(user_id: str, activity: str, details: dict[str, Any] | None = None):
        """記錄可疑活動"""
        MemoryAuditLogger.log_security_event(
...
def audit_log(user_id: str, action: str, resource: str, details: dict[str, Any] | None = None):
    """
    通用審計日誌函數
    ...
    logger.info(f"Audit: {action} on {resource}", extra=audit_entry)
