...
import pytest
from processor_entry import memory_service, process_normalized_message, process_sqs_event
...
class TestMemoryIntegration:
    ...
    def test_process_normalized_message_without_memory(self):
        ...
        normalized = {
            "messageId": "test-123",
            "content": {"text": "你好", "messageType": "text"},
            "user": {"id": "316743844", "displayName": "Test User"},
    ...
    def test_process_normalized_message_with_memory_enabled(self):
        ...
        normalized = {
            "messageId": "test-456",
            "content": {"text": "記住我的名字是 Steven", "messageType": "text"},
            "user": {"id": "316743844", "displayName": "Steven"},
            "context": {"sessionId": "steven-session"},
        }

        # Mock Memory Service
        with (
            patch.object(memory_service, "enabled", True),
    ...
    def test_process_normalized_message_memory_failure_fallback(self):
        ...
        normalized = {
            "messageId": "test-789",
            "content": {"text": "測試容錯", "messageType": "text"},
            "user": {"id": "316743844", "displayName": "Test User"},
            "context": {"sessionId": "test-session"},
        }
        ...
        with (
            patch.object(memory_service, "enabled", True),
            patch.object(
                memory_service, "get_session_manager", side_effect=Exception("Memory error")
            ),
        ):
            # Mock ConversationAgent
            with patch("processor_entry.ConversationAgent") as MockAgent:
    ...
    def test_process_sqs_event_with_memory(self):
        ...
        sqs_event = {
            "Records": [{"body": '{"message": {"from": {"id": 316743844}, "text": "測試 SQS"}}'}]
        }
        ...
        with (
            ...
            with patch("processor_entry.ConversationAgent") as MockAgent:
                mock_agent_instance = Mock()
                mock_agent_instance.process_message.return_value = {"response": "SQS 處理成功"}
                MockAgent.return_value = mock_agent_instance
    ...
    def test_user_id_conversion_to_string(self):
        ...
        normalized = {
            ...
            "content": {"text": "測試", "messageType": "text"},
            "user": {
                "id": 316743844,  # 整數型別
                "displayName": "Test",
            },
        ...
        with patch("processor_entry.ConversationAgent") as MockAgent:
            mock_agent_instance = Mock()
            mock_agent_instance.process_message.return_value = {"response": "測試回應"}
            MockAgent.return_value = mock_agent_instance
    ...
    def test_session_id_defaults_to_user_id(self):
        ...
        normalized = {
            ...
            "content": {"text": "測試", "messageType": "text"},
            "user": {"id": "999888777", "displayName": "Test"},
