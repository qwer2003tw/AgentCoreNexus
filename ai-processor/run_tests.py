...
def run_specific_module(module_name, verbosity=2):
    ...
    print("\n" + "=" * 70)
    print(
        f"完成 - 執行: {result.testsRun}, 失敗: {len(result.failures)}, 錯誤: {len(result.errors)}"
    )
    print("=" * 70)
...
範例:
  ...
  python run_tests.py -q                 # 安靜模式
    ...
    parser.add_argument(
        "-m", "--module", help="指定要執行的測試模組 (例如: test_config)", default=None
    )
    ...
    parser.add_argument("-q", "--quiet", action="store_true", help="安靜模式 (等同於 -v 0)")
