...
## 🛡️ Pre-commit Hook 已實施

**重要通知**：本專案已實施 pre-commit hook，技術上強制執行所有檢查！

### Hook 的作用

Pre-commit hook 會在每次 `git commit` 時自動執行：
...
**即使有 hook 保護，你仍然必須主動執行測試！**

**為什麼？**
1. **第一道防線**：你的主動執行是第一道防線，hook 是備用保險
2. **用戶溝通**：你需要向用戶報告測試結果
3. **提前發現**：提前發現問題比 hook 阻止 commit 更好
4. **專業標準**：這是 AI agent 的專業職責

### 雙重保險機制

```
AI 主動測試 → 發現並修復問題 → 向用戶報告 → 建議 commit
                                              ↓
                                        Hook 驗證 → ✅ 通過
```
...
如果 AI 忘記測試或測試沒通過：
```
AI 跳過測試 → 直接建議 commit → Hook 阻止 → ❌ Commit 失敗
```
