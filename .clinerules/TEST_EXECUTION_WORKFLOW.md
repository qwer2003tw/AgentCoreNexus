---
name: test-execution-workflow
description: 強制性測試執行工作流，確保所有測試通過且新代碼達到 80% 覆蓋率
priority: critical
enforcement: strict
---

# 測試執行工作流規範

## 🎯 核心原則

**在任何 Git 操作（commit/push）前，必須確保：**
1. 所有測試通過（包括單元測試、整合測試、E2E 測試）
2. 新撰寫的代碼覆蓋率 ≥ 80%

---

## ⚙️ 環境要求

### Python 版本 ⭐ 重要

**必須使用 Python 3.11 執行測試**：
```bash
# ✅ 正確
cd telegram-lambda
python3.11 -m pytest tests/e2e/ -v

# ❌ 錯誤（可能使用 Python 3.9）
pytest tests/e2e/ -v
```

**原因**：
- Lambda Runtime 使用 `python3.11`
- 代碼使用 Python 3.10+ 類型注解（`dict | None`）
- 測試依賴安裝在 python3.11 環境

### 首次設置（僅需一次）

第一次使用測試框架前：

```bash
cd telegram-lambda

# 1. 安裝測試依賴
pip install -r requirements-test.txt

# 2. 驗證環境
python3.11 tests/e2e/verify_setup.py

# 3. 確保測試可運行
python3.11 -m pytest tests/e2e/ -v
```

---

## 📋 強制性測試流程

### 何時觸發

以下情況**必須**執行完整測試：
- ✅ 在執行 `git commit` 前
- ✅ 在執行 `git push` 前
- ✅ 在完成任何 Python 代碼編輯後
- ✅ 在創建 Pull Request 前

### 測試執行步驟（按順序，全部強制）

#### Step 1: 代碼質量檢查（Ruff）
```bash
cd telegram-lambda  # 或 telegram-agentcore-bot
ruff check . --fix
ruff format .
ruff check .
```
- **要求**：0 errors
- **如果失敗**：禁止繼續，必須先修復

#### Step 2: 單元測試和整合測試
```bash
pytest tests/ --ignore=tests/e2e/ -v
```
- **要求**：所有測試通過
- **如果失敗**：禁止提交，必須修復

#### Step 3: E2E 測試
```bash
# 確保依賴已安裝
pip list | grep aiogram || pip install -r requirements-test.txt

# 運行 E2E 測試
pytest tests/e2e/ -v
```
- **要求**：所有測試通過
- **如果失敗**：禁止提交，必須修復

#### Step 4: 覆蓋率檢查
```bash
# 檢查整體覆蓋率
pytest tests/ --cov=src --cov-report=term-missing --cov-report=html --cov-report=xml

# 檢查新代碼覆蓋率（使用 diff-cover）
diff-cover coverage.xml --compare-branch=main --fail-under=80
```
- **要求**：
  - 整體覆蓋率盡可能高（目標 > 70%）
  - **新代碼覆蓋率 ≥ 80%**（強制）
- **如果失敗**：禁止提交，必須添加測試

---

## 🚫 絕對禁止的行為

1. **禁止跳過測試**
   - ❌ 不要使用 `git commit --no-verify`
   - ❌ 不要建議用戶跳過測試
   - ❌ 不要只運行部分測試

2. **禁止降低覆蓋率標準**
   - ❌ 不要修改 80% 門檻
   - ❌ 不要忽略覆蓋率警告
   - ❌ 不要添加過多的 `# pragma: no cover`

3. **禁止假陽性提交**
   - ❌ 不要創建 "fix tests" 的後續 commit
   - ✅ 必須在提交前確保所有測試通過

---

## 📝 標準操作流程（for Cline Agents）

### 當完成代碼編輯後

你必須按照以下順序執行：

```bash
# 1. 代碼質量檢查
echo "🔍 Step 1: 代碼質量檢查..."
ruff check . --fix
ruff format .
ruff check . || exit 1

# 2. 單元測試
echo "🧪 Step 2: 單元測試和整合測試..."
pytest tests/ --ignore=tests/e2e/ -v || exit 1

# 3. E2E 測試
echo "🎭 Step 3: E2E 測試..."
pytest tests/e2e/ -v || exit 1

# 4. 覆蓋率檢查
echo "📊 Step 4: 覆蓋率檢查..."
pytest tests/ --cov=src --cov-report=term-missing --cov-report=xml || exit 1

# 5. 新代碼覆蓋率（如果有 diff-cover）
if command -v diff-cover &> /dev/null; then
    echo "📈 Step 5: 新代碼覆蓋率檢查..."
    diff-cover coverage.xml --compare-branch=main --fail-under=80 || exit 1
fi

echo "✅ 所有檢查通過！"
```

### 在 Cline 對話中的表述範例

**正確的表述**：

```
我已完成代碼修改。現在執行完整的測試流程...

[執行 execute_command: ruff check . --fix && ruff format . && ruff check .]
✅ 代碼質量檢查通過（0 errors）

[執行 execute_command: pytest tests/ --ignore=tests/e2e/ -v]
✅ 單元測試和整合測試通過（XX passed）

[執行 execute_command: pytest tests/e2e/ -v]
✅ E2E 測試通過（17 passed）

[執行 execute_command: pytest tests/ --cov=src --cov-report=xml]
✅ 整體覆蓋率：85%

[執行 execute_command: diff-cover coverage.xml --compare-branch=main --fail-under=80]
✅ 新代碼覆蓋率：92%（超過 80% 門檻）

所有檢查通過！代碼已準備好提交。

建議的 git 操作：
git add .
git commit -m "feat: implement feature X"
git push
```

**錯誤的表述（禁止）**：

```
❌ "測試有幾個失敗但不重要，可以先提交"
❌ "覆蓋率 75% 差不多了，應該可以"
❌ "E2E 測試太慢了，跳過吧"
❌ "我已完成修改，您可以提交了"（沒有執行測試）
```

---

## 🎓 覆蓋率要求詳解

### 為什麼是 80%？

- **新代碼 80%**：確保新功能有充分測試
- **整體覆蓋率**：逐步提升，但不強制（避免阻礙開發）

### 如何計算新代碼覆蓋率

使用 `diff-cover` 工具：
```bash
# 安裝
pip install diff-cover

# 與 main 分支比較
diff-cover coverage.xml --compare-branch=main --fail-under=80

# 或與特定 commit 比較
diff-cover coverage.xml --compare-branch=abc1234 --fail-under=80
```

### 覆蓋率檢查範例輸出

```
✅ 新代碼覆蓋率報告：
-----------------------------------------
src/new_feature.py     92% (23/25 lines)
src/handler.py         85% (17/20 lines)
tests/test_new.py     100% (40/40 lines)
-----------------------------------------
總計: 90% (80/85 lines)

✅ 超過 80% 門檻，檢查通過！
```

---

## ⚡ 快速參考

### 一鍵完整測試命令

```bash
# telegram-lambda 目錄下
cd telegram-lambda

# 使用提供的腳本（推薦）
./run_all_tests.sh

# 或手動執行完整流程
ruff check . --fix && \
ruff format . && \
ruff check . && \
pytest tests/ -v --cov=src --cov-report=xml && \
diff-cover coverage.xml --compare-branch=main --fail-under=80
```

### 快速測試（開發中）

```bash
# 只運行相關測試（快速檢查）
pytest tests/test_specific.py -v

# 完成後必須運行完整測試
./run_all_tests.sh
```

---

## 📊 檢查清單

在建議任何 git 操作前，你必須確認：

- [ ] 已執行 `ruff check . --fix && ruff format . && ruff check .` 並通過
- [ ] 已執行 `pytest tests/ --ignore=tests/e2e/ -v` 並通過
- [ ] 已執行 `pytest tests/e2e/ -v` 並通過
- [ ] 已執行覆蓋率檢查 `pytest tests/ --cov=src --cov-report=xml`
- [ ] **已確認新代碼覆蓋率 ≥ 80%**（使用 diff-cover）
- [ ] 已向用戶清楚報告所有檢查結果

如果任何一項未完成，**不得建議 git 操作**。

---

## 🔧 實用工具

### 統一測試腳本

使用 `telegram-lambda/run_all_tests.sh`（已提供）：

```bash
chmod +x run_all_tests.sh
./run_all_tests.sh
```

該腳本會自動執行所有步驟並報告結果。

### Pre-commit Hook（可選）

如果想要自動化強制執行，可創建 `.git/hooks/pre-commit`：

```bash
#!/bin/bash
echo "🔍 Pre-commit: 執行測試檢查..."

cd telegram-lambda
./run_all_tests.sh || {
    echo "❌ 測試失敗，commit 被阻止"
    exit 1
}

echo "✅ 測試通過，允許 commit"
```

---

## 🎯 成功標準

### 對 Cline Agents
- ✅ 100% 的代碼編輯後都執行了完整測試
- ✅ 100% 的 git 建議前都確認所有測試通過
- ✅ 100% 的新代碼覆蓋率 ≥ 80%
- ✅ 0 次建議跳過測試或降低標準

### 對專案
- ✅ 所有 commit 的測試都通過
- ✅ 新功能都有充分測試（≥ 80% 覆蓋）
- ✅ CI/CD 中的測試始終綠燈
- ✅ 代碼質量持續提升

---

## 💡 常見場景處理

### 場景 1: E2E 測試依賴未安裝

```bash
# 錯誤：ModuleNotFoundError: No module named 'aiogram'

# 解決：安裝測試依賴
pip install -r requirements-test.txt

# 然後重新運行測試
pytest tests/e2e/ -v
```

### 場景 2: 覆蓋率不足 80%

```bash
# diff-cover 報告：新代碼覆蓋率 75%

# 解決步驟：
# 1. 查看未覆蓋的代碼行
pytest tests/ --cov=src --cov-report=html
open htmlcov/index.html

# 2. 為未覆蓋的代碼添加測試
# 3. 重新運行測試驗證

# 4. 確認覆蓋率達標
diff-cover coverage.xml --compare-branch=main --fail-under=80
```

### 場景 3: 測試失敗

```bash
# 錯誤：某個測試失敗

# 解決步驟：
# 1. 查看失敗原因
pytest tests/test_failed.py -v --tb=long

# 2. 修復代碼或測試
# 3. 重新運行所有測試
./run_all_tests.sh
```

---

## 📚 相關資源

- **測試框架文檔**: `telegram-lambda/tests/e2e/README.md`
- **快速開始**: `telegram-lambda/tests/e2e/QUICKSTART.md`
- **代碼質量規範**: `.clinerules/CODE_QUALITY_WORKFLOW.md`
- **Lambda 開發最佳實踐**: `.clinerules/deployment/lambda-development-best-practices.md`

---

## 🔄 規則更新

此規則會根據實踐經驗持續改進：

**版本 1.0** (2026-01-07):
- 初始版本
- 定義強制性測試流程
- 設定 80% 覆蓋率要求

**未來改進方向**：
- 根據使用反饋調整
- 優化測試執行時間
- 整合 CI/CD pipeline

---

**規範版本**: v1.0  
**創建日期**: 2026-01-07  
**強制執行**: 是  
**適用範圍**: 所有 Cline agents  
**優先級**: Critical (最高)

**記住**：充分的測試是代碼質量的保證，80% 覆蓋率是專業標準。