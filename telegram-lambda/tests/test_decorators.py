...
class TestRequireAdmin:
    ...
    def test_admin_permission_granted(
        self, mock_send, mock_check, mock_update, mock_event
    ):
        """測試管理員權限通過"""
    ...
    @patch('commands.decorators.check_permission')
    @patch('commands.decorators.send_permission_denied')
    def test_admin_permission_denied(
        self, mock_send, mock_check, mock_update, mock_event
    ):
        """測試管理員權限被拒絕"""
...
class TestRequireAllowlist:
    ...
    @patch('commands.decorators.check_permission')
    @patch('commands.decorators.send_permission_denied')
    def test_allowlist_permission_granted(
        self, mock_send, mock_check, mock_update, mock_event
    ):
        """測試 allowlist 權限通過"""
        mock_check.return_value = True
    ...
    @patch('commands.decorators.check_permission')
    @patch('commands.decorators.send_permission_denied')
    def test_allowlist_permission_denied(
        self, mock_send, mock_check, mock_update, mock_event
    ):
        """測試 allowlist 權限被拒絕"""
        mock_check.return_value = False
...
class TestRequirePermission:
    ...
    @patch('commands.decorators.check_permission')
    @patch('commands.decorators.send_permission_denied')
    def test_custom_permission_level(
        self, mock_send, mock_check, mock_update, mock_event
    ):
        """測試自訂權限等級"""
        mock_check.return_value = True
    ...
    @patch('commands.decorators.check_permission')
    @patch('commands.decorators.send_permission_denied')
    def test_none_permission_always_passes(
        self, mock_send, mock_check, mock_update, mock_event
    ):
        """測試 NONE 權限總是通過"""
        mock_check.return_value = True
...
class TestDecoratorEdgeCases:
    ...
    def test_no_message_in_update(
        self, mock_send, mock_check, mock_event
    ):
        """測試 Update 中沒有訊息"""
        update = Mock(spec=Update)
        update.message = None
