...
def check_permission(
    ...
    if required == Permission.NONE:
        logger.debug(
            f"Permission check: NONE - always granted",
            extra={
                'chat_id': chat_id,
                'username': username,
    ...
    if is_admin(chat_id, username):
        logger.info(
            f"Permission check: granted (admin)",
            extra={
                'chat_id': chat_id,
                'username': username,
    ...
    if required == Permission.ALLOWLIST:
        ...
        if allowed:
            logger.info(
                f"Permission check: granted (allowlist)",
                extra={
                    'chat_id': chat_id,
                    'username': username,
            ...
            logger.warning(
                f"Permission check: denied (not in allowlist)",
                extra={
                    'chat_id': chat_id,
                    'username': username,
                    'required_permission': 'ALLOWLIST',
    ...
    # 需要 admin 但不是 admin
    logger.warning(
        f"Permission check: denied (requires admin)",
        extra={
            'chat_id': chat_id,
            'username': username,
