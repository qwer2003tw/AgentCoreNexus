...
def main():
    ...
    for i, user_id in enumerate(user_ids, 1):
        ...
        try:
            conv_count, msg_count = migrate_user_conversations(user_id, dry_run=args.dry_run)
...
def get_all_user_ids() -> list[str]:
    ...
    try:
        # Scan bindings table
        response = bindings_table.scan(ProjectionExpression="unified_user_id")
        ...
        for msg in conv["messages"]:
            try:
                history_table.update_item(
                    Key={"unified_user_id": user_id, "timestamp_msgid": msg["timestamp_msgid"]},
