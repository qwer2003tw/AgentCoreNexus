...
def handle_bind_command(chat_id: int, username: str, args: list[str]) -> dict[str, Any]:
    ...
    if not BINDINGS_TABLE or not BINDING_CODES_TABLE:
        return {"success": False, "message": "❌ Binding功能尚未啟用"}

    # Check if code is provided
    if not args or len(args) == 0:
        return {"success": False, "message": "❌ 請提供綁定碼\n\n使用方式: /bind 123456"}

    code = args[0].strip()

    # Validate code format (6 digits)
    if not code.isdigit() or len(code) != 6:
        return {"success": False, "message": "❌ 綁定碼格式錯誤\n\n綁定碼應為 6 位數字"}
    ...
    try:
        ...
        if existing_binding:
            return {
                "success": False,
                "message": "❌ 此 Telegram 帳號已綁定至另一個帳戶\n\n如需重新綁定，請聯絡管理員",
            }
        ...
        traceback.print_exc()
        return {"success": False, "message": "❌ 綁定過程發生錯誤\n\n請稍後再試或聯絡管理員"}
...
def update_binding_with_telegram(
    unified_user_id: str, telegram_chat_id: int, web_email: str
) -> None:
    """
