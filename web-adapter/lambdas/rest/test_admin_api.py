...
sys.modules["audit_decorator"] = MagicMock(
    audit_log=audit_log, require_permission=require_permission
)
...
def dynamodb_tables():
    ...
    with mock_aws():
        ...
        yield {"history": history_table, "summaries": summaries_table, "dynamodb": dynamodb}
        ...
        history_table.put_item(
            Item={
                ...
                "content": {  # Web format: object
                    "text": "這是什麼？",
                    "attachments": [{"type": "document", "file_name": "doc.pdf"}],
                },
            }
        )
...
class TestGenerateSummary:
    ...
    def test_generate_new_summary(self, mock_bedrock, dynamodb_tables):
        ...
        mock_bedrock.invoke_model.return_value = {
            "body": Mock(read=lambda: json.dumps({"content": [{"text": "測試摘要內容"}]}).encode())
        }
