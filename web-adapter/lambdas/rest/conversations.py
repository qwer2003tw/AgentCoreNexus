...
def handle_list_conversations(email: str, event: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        response_data = {
            "conversations": {"pinned": pinned, "recent": unpinned},
...
def handle_update_conversation(email: str, conv_id: str, event: dict[str, Any]) -> dict[str, Any]:
    """
    更新對話（重命名、置頂）
...
def handle_delete_conversation(email: str, conv_id: str) -> dict[str, Any]:
    ...
    try:
        conversations_table.update_item(
            ...
            ExpressionAttributeValues={":true": True, ":now": datetime.now(UTC).isoformat()},
...
def handle_get_messages(email: str, conv_id: str, event: dict[str, Any]) -> dict[str, Any]:
    """
    獲取特定對話的所有消息
    ...
    try:
        ...
        query_kwargs = {
            ...
            "ExpressionAttributeValues": {":user_id": unified_user_id, ":conv_id": conv_id},
            "Limit": limit,
        ...
        response_data = {"messages": messages, "count": len(messages)}
