...
def create_memory():
    ...
    try:
        ...
        strategies = [
            UserPreferenceStrategy(
                name="userPreferences", namespaces=["/actors/{actorId}/preferences"]
            ),
            SemanticStrategy(name="userFacts", namespaces=["/actors/{actorId}/facts"]),
            SummaryStrategy(
                name="sessionSummaries", namespaces=["/actors/{actorId}/sessions/{sessionId}"]
            ),
        ]
        ...
        if memory.get("status") != "ACTIVE":
            ...
            while waited < max_wait:
                ...
                current_memory = next(
                    (m for m in memories if m.get("id") == memory.get("id")), None
                )
        ...
        print(f"  --region {region} \\")
        print("  --function-name telegram-unified-bot-processor \\")
        print('  --environment "Variables={')
        print(f"    BEDROCK_AGENTCORE_MEMORY_ID={memory.get('id')},")
        print("    EVENT_BUS_NAME=telegram-lambda-receiver-events,")
        print("    BEDROCK_MODEL_ID=anthropic.claude-3-5-sonnet-20241022-v2:0,")
        print("    BROWSER_ENABLED=true,")
        print("    LOG_LEVEL=INFO")
        print('  }"')
        print()
        print("3. 等待 Lambda 更新完成：")
        print()
        print("aws lambda wait function-updated \\")
        print(f"  --region {region} \\")
        print("  --function-name telegram-unified-bot-processor")
        print()
        print("=" * 80)
...
if __name__ == "__main__":
    ...
    print()
    print("💾 Memory ID 已儲存到：MEMORY_ID.txt")
