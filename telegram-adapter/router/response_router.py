...
from router.delivery import DeliveryResult, TelegramDelivery
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        if missing_fields:
            ...
            logger.error(
                error_msg,
                extra={"event_type": "router_invalid_event", "missing_fields": missing_fields},
            )
        ...
        result = route_message(
            channel=channel, user_id=user_id, content=response_content, metadata=metadata
        )
        ...
        if result.success:
            ...
            return {
                "statusCode": 500,
                "body": json.dumps(
                    {"success": False, "messageId": message_id, "error": result.error}
                ),
            }
        ...
        logger.error(
            ...
            extra={"event_type": "router_error", "duration_ms": duration_ms},
            exc_info=True,
        )
...
def route_message(
    channel: str, user_id: str, content: str, metadata: dict[str, Any] | None = None
) -> DeliveryResult:
    """
    路由訊息到指定頻道
    ...
    if channel not in DELIVERY_MAP:
        ...
        return DeliveryResult(success=False, channel=channel, user_id=user_id, error=error_msg)
    ...
    try:
        ...
        context = {"parse_mode": formatter.get_parse_mode()}
        if metadata:
            context.update(metadata)

        # 傳送訊息
        result = delivery.deliver(user_id=user_id, message=formatted_content, context=context)
        ...
        logger.error(
            error_msg,
            extra={"event_type": "router_exception", "channel": channel, "user_id": user_id},
            exc_info=True,
        )
