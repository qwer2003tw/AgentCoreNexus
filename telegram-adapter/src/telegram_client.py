...
def send_message(chat_id: int, text: str, parse_mode: str | None = None) -> bool:
    """
    發送訊息到 Telegram (同步包裝)
    ...
    try:
        ...
        logger.error(
            ...
            extra={"chat_id": chat_id, "event_type": "telegram_send_error"},
            exc_info=True,
        )
        return False


async def _send_message_async(chat_id: int, text: str, parse_mode: str | None = None) -> bool:
    """
    異步發送訊息到 Telegram
    ...
    try:
        ...
        # 發送訊息
        message = await bot.send_message(chat_id=chat_id, text=text, parse_mode=telegram_parse_mode)
...
async def _send_long_message_async(
    bot: Bot, chat_id: int, text: str, parse_mode: str | None
) -> bool:
    """
    發送長訊息（自動分段）
    ...
    for i, chunk in enumerate(chunks, 1):
        try:
            await bot.send_message(
                chat_id=chat_id, text=f"📄 Part {i}/{len(chunks)}\n\n{chunk}", parse_mode=parse_mode
            )
        except TelegramError as e:
            all_success = False
            logger.warning(
                f"Failed to send chunk {i}/{len(chunks)}: {str(e)}",
                extra={"chat_id": chat_id, "chunk_index": i, "event_type": "chunk_send_failed"},
            )
...
def send_debug_info(chat_id: int, event: dict) -> bool:
    ...
    try:
        ...
        logger.error(
            ...
            extra={"chat_id": chat_id, "event_type": "debug_format_error"},
            exc_info=True,
        )
        return False
