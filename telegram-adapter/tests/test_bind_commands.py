...
class TestBindCommandHandler:
    ...
    def test_can_handle_bind_command(self):
        ...
        assert handler.can_handle("/bind") == True
        assert handler.can_handle("/bind ") == True
        assert handler.can_handle(" /bind") == True
        assert handler.can_handle("/mybindings") == False
        assert handler.can_handle("hello") == False

    @patch('commands.handlers.bind_handler.telegram_client')
    @patch('commands.handlers.bind_handler.os.getenv')
    def test_handle_bind_success(self, mock_getenv, mock_telegram_client, mock_identity_service, mock_update):
        """測試成功生成綁定碼"""
        # Setup
    ...
    @patch('commands.handlers.bind_handler.telegram_client')
    @patch('commands.handlers.bind_handler.os.getenv')
    def test_handle_bind_generation_error(self, mock_getenv, mock_telegram_client, mock_identity_service, mock_update):
        """測試綁定碼生成失敗"""
        # Setup
        mock_getenv.side_effect = lambda k: {
            'BINDING_CODES_TABLE': 'test-binding-codes',
...
class TestMyBindingsCommandHandler:
    ...
    def test_can_handle_mybindings_command(self):
        ...
        assert handler.can_handle("/mybindings") == True
        assert handler.can_handle("/mybindings ") == True
        assert handler.can_handle("/bind") == False

    @patch('commands.handlers.mybindings_handler.telegram_client')
    @patch('commands.handlers.mybindings_handler.os.getenv')
    def test_handle_mybindings_no_bindings(self, mock_getenv, mock_telegram_client, mock_identity_service, mock_update):
        """測試沒有綁定時的回應"""
        # Setup
        mock_getenv.side_effect = lambda k: {
            'BINDING_CODES_TABLE': 'test-binding-codes',
    ...
    @patch('commands.handlers.mybindings_handler.telegram_client')
    @patch('commands.handlers.mybindings_handler.os.getenv')
    def test_handle_mybindings_with_bindings(self, mock_getenv, mock_telegram_client, mock_identity_service, mock_update):
        """測試有綁定時的回應"""
        # Setup
        mock_getenv.side_effect = lambda k: {
            'BINDING_CODES_TABLE': 'test-binding-codes',
...
class TestUnbindCommandHandler:
    ...
    def test_can_handle_unbind_commands(self):
        ...
        assert handler.can_handle("/unbind") == True
        assert handler.can_handle("/unbind confirm") == True
        assert handler.can_handle("/unbind ") == True
        assert handler.can_handle("/bind") == False

    @patch('commands.handlers.unbind_handler.telegram_client')
    @patch('commands.handlers.unbind_handler.os.getenv')
    def test_handle_unbind_no_bindings(self, mock_getenv, mock_telegram_client, mock_identity_service):
        """測試沒有綁定時的 unbind"""
        # Setup
        mock_getenv.side_effect = lambda k: {
            'BINDING_CODES_TABLE': 'test-binding-codes',
    ...
    @patch('commands.handlers.unbind_handler.telegram_client')
    @patch('commands.handlers.unbind_handler.os.getenv')
    def test_handle_unbind_confirmation_step(self, mock_getenv, mock_telegram_client, mock_identity_service):
        """測試 unbind 確認步驟"""
        # Setup
        mock_getenv.side_effect = lambda k: {
            'BINDING_CODES_TABLE': 'test-binding-codes',
    ...
    @patch('commands.handlers.unbind_handler.telegram_client')
    @patch('commands.handlers.unbind_handler.os.getenv')
    def test_handle_unbind_confirm_success(self, mock_getenv, mock_telegram_client, mock_identity_service):
        ...
        user = User(id=316743844, first_name="Test", is_bot=False, username="testuser")
        chat = Chat(id=316743844, type="private")
        message = Message(message_id=1, date=None, chat=chat, from_user=user, text="/unbind confirm")
        update = Update(update_id=1, message=message)
...
class TestCommandIntegration:
    ...
    @patch('commands.handlers.bind_handler.os.getenv')
    @patch('commands.handlers.mybindings_handler.os.getenv')
    def test_bind_then_check_flow(self, mock_getenv_mybindings, mock_getenv_bind,
                                   mock_telegram_mybindings, mock_telegram_bind,
        ...
        mybindings_message = Message(message_id=2, date=None, chat=chat, from_user=user, text="/mybindings")
