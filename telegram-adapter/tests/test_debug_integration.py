...
from src.telegram_client import SENSITIVE_FIELDS, send_debug_info
...
def test_debug_command_with_logging(caplog, monkeypatch):
    ...
    test_event = {
        ...
        "multiValueHeaders": {"X-Telegram-Bot-Api-Secret-Token": ["test_secret_token_12345"]},
    ...
    with patch("src.telegram_client.send_message") as mock_send:
        ...
        for _i, record in enumerate(caplog.records):
...
def test_debug_command_redaction_fields():
    ...
    test_event = {
        ...
        "requestContext": {"accountId": "190825685292", "stage": "Prod"},
    }
        ...
        for path in SENSITIVE_FIELDS:
            ...
            if found:
                if isinstance(current, list):
                    status = (
                        "✅ 已遮蔽 (列表)"
                        if all(v == "[REDACTED]" for v in current)
                        else "❌ 未遮蔽"
                    )
                else:
                    status = "✅ 已遮蔽" if current == "[REDACTED]" else "❌ 未遮蔽"
                print(f"  {field_name}: {status}")
                assert current == "[REDACTED]" or (
                    isinstance(current, list) and all(v == "[REDACTED]" for v in current)
                )
            else:
