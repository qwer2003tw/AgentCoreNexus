...
def mock_secrets_response():
    ...
    return {
        "SecretString": json.dumps(
            {"bot_token": "test-bot-token-123", "webhook_secret_token": "test-webhook-token-456"}
        )
    }
...
def mock_env_vars(monkeypatch):
    """設定測試環境變數"""
    monkeypatch.setenv(
        "TELEGRAM_SECRETS_ARN",
        "arn:aws:secretsmanager:us-west-2:123456789012:secret:telegram-secrets",
    )
...
class TestGetSecret:
    ...
    def test_get_secret_invalid_json(self, mock_client):
        """測試無效 JSON 處理"""
        mock_sm = MagicMock()
        mock_sm.get_secret_value.return_value = {"SecretString": "invalid json"}
        mock_client.return_value = mock_sm
...
class TestGetTelegramSecrets:
    ...
    def test_get_telegram_secrets_missing_bot_token(self, mock_get_secret, mock_env_vars):
        """測試缺少 bot_token"""
        mock_get_secret.return_value = {"webhook_secret_token": "webhook-token-456"}
    ...
    def test_get_telegram_secrets_missing_webhook_token(self, mock_get_secret, mock_env_vars):
        ...
        mock_get_secret.return_value = {"bot_token": "bot-token-123"}
...
class TestGetTelegramBotToken:
    ...
    def test_get_bot_token_no_token_field(self, mock_get_secrets):
        ...
        mock_get_secrets.return_value = {"webhook_secret_token": "webhook-token-456"}
...
class TestGetTelegramSecretToken:
    ...
    def test_get_secret_token_no_token_field(self, mock_get_secrets):
        ...
        mock_get_secrets.return_value = {"bot_token": "bot-token-123"}
